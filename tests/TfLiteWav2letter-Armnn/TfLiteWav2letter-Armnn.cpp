...
static int image_pre_process(const char* file, void* data, uint32_t size)
{
...
static int image_post_process(const char* file, void* data, uint32_t size)
{
    int8_t* ptr = reinterpret_cast<int8_t*>(data);
    for(uint32_t i =0; i < size; i++)
    {
...
int main(int argc, char* argv[])
{
    ...
    std::vector<std::vector<int8_t>> in;
    for (unsigned int i = 0 ; i < nb_inputs ; i++)
    {
        std::vector<int8_t> in_data(inputTensorInfos.at(i).GetNumElements());
    ...
    {
        std::vector<int8_t> out_data(outputTensorInfos.at(i).GetNumElements());
    ...
    for (int i = 0 ; i < nb_loops ; i++)
    {
        ...
        image_pre_process(input_file_str.c_str(), in[0].data(), in[0].size());
        runtime->EnqueueWorkload(networkId, inputTensors, outputTensors);

        gettimeofday(&stop_time, nullptr);
        image_post_process(input_file_str.c_str(), out[0].data(), out[0].size());
